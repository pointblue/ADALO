...
#load the breeding or winter data
	...
	if(!is.null(brtm)){
		...
		nt<-brtm$n.trees	
		m5data$predicted_gbm<-predict(brtm,n.trees=nt)
		
		#predict to covars
		preddf<-covarsdf
		preddf$predicted<-predict(brtm,preddf,n.trees=nt)
		cid<-as.integer(preddf$cellId);
		if(uselog==TRUE){
			civ<-exp(preddf$predicted)-minv
		}else{
			civ<-preddf$predicted
		}
		civ<-ifelse(civ<0,0,civ)
...
save(mdlLst,file=paste(rpth,sesfldr,"/m5dataNew2",suffgeo,sufflog,"/mdlLst_",seas,".RData",sep=""))


##############################################################################################
#tinkering with model fitting parameters...
