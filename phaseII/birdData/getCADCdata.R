...
processSUIDs<-function(sss,con2,con3,con4,rail="no"){
	...
	if(NROW(df.pointgeo)>0){
		...
		df.pointgeo<-na.omit(df.pointgeo)
		if(rail=="no"){
			df.pointgeo<-subset(df.pointgeo,grepl("-CA-|-NV-|-OR-|-AZ-|-MT-|-ND-|-SD-|-WY-|-CO-|-UT-|-NE-|-KS-|-NM-",county_code))
		}else{
			df.pointgeo<-subset(df.pointgeo,grepl("-CA-|-AZ-",county_code))
		}
...
getSpeciesData<-function(pointgeo.df,con1,spp.df,dtype){
	if(NROW(pointgeo.df)>0){
		...
		for(ccc in spcodes){
			...
			sus<-pointgeo.df
			if(ccc!="WIFL"){
				sus<-subset(sus,grepl("-CA-|-NV-|-OR-|-AZ-|-MT-|-ND-|-SD-|-WY-|-CO-|-UT-|-NE-|-KS-|-NM-",county_code),select="SamplingUnitId")
			}
			...
			if(dtype=="AS"){
				...
				filen<-paste("/home/lsalas/refuge22/cadc_as_",ccc,".RData",sep="")
			}else{
				...
				filen<-paste("/home/lsalas/refuge22/cadc_pc_",ccc,".RData",sep="")
			}
...
#Species
spp.df<-data.frame(
		commonName=c("Black Rail","Ridgway\\'s Rail","Brandt\\'s Cormorant","Least Tern","Canvasback","Sandhill Crane","Long-billed Curlew",
		...
		spcd=c("BLRA","RIRA","BRCO","LETE","CANV","SACR","LBCU","NOPI","WIFL","TRBL","SNPL","MOPL",
...
effsql<-paste(
		...
		"from ebd where",
		"(STATE_CODE in ('US-CA','US-NV','US-OR','US-MT','US-ND','US-SD','US-WY','US-CO','US-UT','US-NE','US-KS'))",
		"and (APPROVED = 1)",
...
effsql.wifl<-paste(
		...
		"SAMPLING_EVENT_IDENTIFIER, OBSERVATION_DATE, TIME_OBSERVATIONS_STARTED, DURATION_MINUTES, NUMBER_OBSERVERS, GROUP_IDENTIFIER, LATITUDE, LONGITUDE",
		"from ebd where",
		"(STATE_CODE in ('US-CA','US-NV','US-OR','US-AZ','US-NM','US-MT','US-ND','US-SD','US-WY','US-CO','US-UT','US-NE','US-KS'))",
		"and (APPROVED = 1)",
		"and (PROTOCOL_TYPE in ('eBird - Exhaustive Area Count', 'eBird - Traveling Count', 'eBird - Stationary Count', 'EBIRD', 'Audubon NWR Protocol', 'EBIRD_CA','TNC California Waterbird Count'))")
...
for(sss in 1:(nrow(spp.df))){
	...
	ebird.obs$ObsCount<-as.numeric(as.character(ebird.obs$ObsCount))
	
	#merge with effort
	if(spec=="Clapper Rail"){
		ebird.mrg<-merge(ebird.eff.clra,ebird.obs,by=c("SAMPLING_EVENT_IDENTIFIER","OBSERVATION_DATE","LATITUDE","LONGITUDE"),all.x=TRUE)
		effort<-ebird.eff.clra
	}else if(spec=="Willow Flycatcher"){
		ebird.mrg<-merge(ebird.eff.wifl,ebird.obs,by=c("SAMPLING_EVENT_IDENTIFIER","OBSERVATION_DATE","LATITUDE","LONGITUDE"),all.x=TRUE)
		effort<-ebird.eff.wifl
	}else{
		ebird.mrg<-merge(ebird.eff,ebird.obs,by=c("SAMPLING_EVENT_IDENTIFIER","OBSERVATION_DATE","LATITUDE","LONGITUDE"),all.x=TRUE)
		effort<-ebird.eff
	}
	...
	filen<-paste("/home/lsalas/refuge22/ebird_",spcd,".RData",sep="")
...
#############################
# BBS
con<-connNodeDb(dsn="ravian_wh")

effsql<-paste("SELECT distinct Transect, SamplingUnitId, DecimalLongitude, DecimalLatitude,YearCollected, MonthCollected, DayCollected, Time",
		"from ravianbbsbase_v1 where (DecimalLatitude is not null) and (DecimalLongitude is not null);")
...
for(sss in 1:(nrow(spp.df))){
	...
	spcd<-spp.df[sss,"spcd"]
	obssql<-paste(
		"SELECT Transect, SamplingUnitId, DecimalLongitude, DecimalLatitude,YearCollected, MonthCollected, DayCollected, Time, CommonName, SpeciesCode, ObservationCount ",
		"from ravianbbsbase_v1 where (DecimalLatitude is not null) and (DecimalLongitude is not null) ",
		"and (CommonName = '",spec,"');",sep="")
	
	bbs.obs<-sqlQuery(con,obssql,rows_at_time=1)
	bbs.mrg<-merge(bbs.eff,bbs.obs,by=c("Transect","SamplingUnitId","DecimalLongitude","DecimalLatitude","YearCollected","MonthCollected","DayCollected","Time"),all.x=T)
	bbs.mrg$obsCount<-ifelse(is.na(bbs.mrg$ObservationCount),0,bbs.mrg$ObservationCount)
	bbs.mrg$CommonName<-spec
	bbs.mrg$SpeciesCode<-spcd
	bbs.mrg$source<-"bbs"
	filen<-paste("/home/lsalas/refuge22/bbs_",spcd,".RData",sep="")
...
df.filtsus.blra<-data.frame()
for(sss in unique(df.suids$SamplingUnitID)){
	fltsu<-processSUIDs(sss=sss,con2=con2,con3=con3,con4=con4,rail="no")
	df.filtsus.blra<-rbind(df.filtsus.blra,fltsu)
}
susstr.blra<-paste(df.filtsus.blra$SamplingUnitId,collapse=",")

df.filtsus.rira<-data.frame()
for(sss in unique(df.suids$SamplingUnitID)){
	fltsu<-processSUIDs(sss=sss,con2=con2,con3=con3,con4=con4,rail="yes")
	df.filtsus.rira<-rbind(df.filtsus.rira,fltsu)
}
susstr.rira<-paste(df.filtsus.rira$SamplingUnitId,collapse=",")
...
#this needs to be done separately for BLRA and RIRA
for(ppp in df.proj$ProjectId){
	if(ppp=="SFBAYRAILS"){
		sqleff.blra<-paste("select distinct ProjectId,SamplingUnitId,ProtocolId,SamplingEventTypeCd,SamplingEventCounter,SamplingEventDt,SamplingEventStartTm from SFBAYRAILS.SamplingEvent_RailCount where SamplingEventTypeCd='PointCount' and samplingUnitId in (",susstr.blra,");",sep="")
		rail.eff.blra<-getRavianWH(con1=con1,sql=sqleff.blra)
		sqleff.rira<-paste("select distinct ProjectId,SamplingUnitId,ProtocolId,SamplingEventTypeCd,SamplingEventCounter,SamplingEventDt,SamplingEventStartTm from SFBAYRAILS.SamplingEvent_RailCount where SamplingEventTypeCd='PointCount' and samplingUnitId in (",susstr.rira,");",sep="")
		rail.eff.rira<-getRavianWH(con1=con1,sql=sqleff.rira)
		#get obs, merge, append to dfs...
		for(sss in railspp.df$spcd){	
			if(sss=="BLRA"){
				fbid<-"263"
				sqlobs<-paste("select ProjectId,SamplingUnitId,ProtocolId,SamplingEventTypeCd,SamplingEventCounter,BirdCountMax from SFBAYRAILS.SamplingEventObservation_RailCount where SamplingEventTypeCd='PointCount' and samplingUnitId in (",susstr.blra,") and FieldBirdId in (",fbid,");",sep="")
				rail.eff<-rail.eff.blra
			}else{
				fbid<-"464,2837"
				sqlobs<-paste("select ProjectId,SamplingUnitId,ProtocolId,SamplingEventTypeCd,SamplingEventCounter,BirdCountMax from SFBAYRAILS.SamplingEventObservation_RailCount where SamplingEventTypeCd='PointCount' and samplingUnitId in (",susstr.rira,") and FieldBirdId in (",fbid,");",sep="")
				rail.eff<-rail.eff.rira
			}
			...
			filen<-paste("/home/lsalas/refuge22/sfb_rail_",sss,".RData",sep="")
			save(rail.eff,rail.obs,rail.mrg,file=filen)
		}
		
	}else{
		
		sqleff.rira<-paste("select distinct ProjectId,SamplingUnitId,ProtocolId,SamplingEventTypeCd,SamplingEventCounter,SamplingEventDt,SamplingEventStartTm from ",ppp,".SamplingEvent_Count where SamplingEventTypeCd='MarshBirdCount' and samplingUnitId in (",susstr.rira,");",sep="")
		sqleff.blra<-paste("select distinct ProjectId,SamplingUnitId,ProtocolId,SamplingEventTypeCd,SamplingEventCounter,SamplingEventDt,SamplingEventStartTm from ",ppp,".SamplingEvent_Count where SamplingEventTypeCd='MarshBirdCount' and samplingUnitId in (",susstr.blra,");",sep="")
		rail.eff.rira<-getRavianWH(con1=con1,sql=sqleff.rira)
		rail.eff.blra<-getRavianWH(con1=con1,sql=sqleff.blra)
		#get obs, merge, append to dfs...
		if(nrow(rail.eff.rira)>0 || nrow(rail.eff.blra)>0){
			for(sss in railspp.df$spcd){	
				if(sss=="BLRA"){
					fbid<-"263"
					sqlobs<-paste("select ProjectId,SamplingUnitId,ProtocolId,SamplingEventTypeCd,SamplingEventCounter,BirdCount from ",ppp,".SamplingEventObservation_Count where SamplingEventTypeCd='MarshBirdCount' and samplingUnitId in (",susstr.blra,") and FieldBirdId in (",fbid,");",sep="")
					rail.eff<-rail.eff.blra
				}else{
					fbid<-"464,2837"
					sqlobs<-paste("select ProjectId,SamplingUnitId,ProtocolId,SamplingEventTypeCd,SamplingEventCounter,BirdCount from ",ppp,".SamplingEventObservation_Count where SamplingEventTypeCd='MarshBirdCount' and samplingUnitId in (",susstr.rira,") and FieldBirdId in (",fbid,");",sep="")
					rail.eff<-rail.eff.rira
				}
				rail.obs<-getRavianWH(con1=con1,sql=sqlobs)
				if(nrow(rail.obs)>0){
					...
					filen<-paste("/home/lsalas/refuge22/",ppp,"_rail_",sss,".RData",sep="")
					save(rail.eff,rail.obs,rail.mrg,file=filen)
				}else{
					if(nrow(rail.eff)>0){
						rail.mrg<-rail.eff
